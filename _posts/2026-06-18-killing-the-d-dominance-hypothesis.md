...
Future work follows directly, and the literature names the first thing to try. For a GAN that looks data-bound, the standard remedy is **augmentation applied to *both* real and fake images** — DiffAugment (Zhao et al., NeurIPS 2020) and StyleGAN2-ADA (Karras et al., NeurIPS 2020), which cut FFHQ-1k FID from ~100 to ~21. A trap this repo invites: the dataloader's commented-out `RandomHorizontalFlip` / `ColorJitter` / `RandomAffine` are the *wrong* fix — augmenting reals only breaks the GAN equilibrium (DiffAugment reports it made CIFAR-10 BigGAN FID *worse*, ~9.6 → up to ~49). The honest counter-caveat is that augmentation's gains shrink at extreme scarcity, so at 2,490 images the outcome is genuinely uncertain — which makes it the next experiment worth *running*, not assuming. That is the subject of the next post, which answers it: ["Can DiffAugment Break the FID-160 Ceiling?"]({% post_url 2026-06-19-can-diffaugment-break-the-fid-ceiling %})
