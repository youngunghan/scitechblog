...
3.  **Count**: Verify the expected color from parity relative to the window origin: `((row - i) + (col - j)) % 2`.
    ...
    for j in range(M - 7):
        ...
        for x in range(i, i + 8):
            for y in range(j, j + 8):
                ...
                # Use coordinates relative to this 8x8 window so that
                # "starts with W/B" always refers to board[i][j].
                is_even_offset = ((x - i) + (y - j)) % 2 == 0
                
                # Case 1: Start with 'W' (Even offset -> W, Odd offset -> B)
                if is_even_offset:
...
| **Chessboard Pattern** | Use parity relative to the window origin, `((r-start_r)+(c-start_c)) % 2`, to check the pattern |
