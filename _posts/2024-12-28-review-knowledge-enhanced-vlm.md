...
>
> **Code**: [official implementation](https://github.com/xiaoman-zhang/KAD).
>
...
Generic medical VLMs such as **CLIP** and **ConVIRT** align images with raw reports but ignore structured medical knowledge, while supervised baselines like **CheXNet** need per-pathology labels. KAD's contribution is injecting a **UMLS knowledge graph** (with RadGraph entity extraction) into pre-training, enabling knowledge-aware zero-shot diagnosis that neither line offers.
...
_Figure 2: Qualitative results showing the attention maps generated by KAD. The model accurately localizes the pathology (red regions) corresponding to the ground truth (red boxes), providing explainability (from Fig. 6 of the paper)._
