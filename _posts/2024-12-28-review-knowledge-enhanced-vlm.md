...
Especially in terms of **"Explainable AI (XAI),"** the fact that the Attention Map generated by the Disease Query Network aligns well with the actual lesion location may help clinicians inspect model behavior—though clinical trust still requires prospective validation, calibration, and workflow evaluation. Future medical VLM research will likely focus not just on increasing data volume, but on **how to effectively integrate high-quality domain knowledge** like this.
