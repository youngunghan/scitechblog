...
> **범위:** 트러블슈팅 글의 증상·원인·해결 구조와 검증 방식. 공통 규칙은 [write-posts.md](../write-posts.md) 참고.
> **대상:** 블로그 작성자.
> **상태:** 구현 반영 — 기준일 2026-07-10.

실제 블로그 글은 영어로 작성합니다. 해결책은 재현 가능한 환경 정보와 공식 출처를 포함해야 합니다.
...
- 강한 problem report는 **Expected / Actual / Reproduction / Environment**를 함께 보입니다([Google SRE — Effective Troubleshooting](https://sre.google/sre-book/effective-troubleshooting/)가 기대·실제 동작과 재현을 문제 보고의 핵심으로 봄). 아래 템플릿의 Problem 1이 이 필드를 제공합니다.
...
일반 규칙은 [write-posts.md §2 Front matter](../write-posts.md#2-front-matter)를 따릅니다. 트러블슈팅 글에서 주의할 점만:
...
author: seoultech
# math: true     # 수식 있을 때만 주석 해제
# mermaid: true  # 다이어그램 있을 때만 주석 해제
---
```

> 본문 인라인 이미지는 front matter와 반대로 **맨 앞에 `/`**를 붙입니다: `![alt](/assets/img/posts/troubleshooting/<filename>.png)`(여기 `<filename>`은 placeholder). 배경은 [write-posts.md §4 이미지와 수식](../write-posts.md#4-이미지와-수식) 참고.
...
- **Reproduction:** <minimal steps or command to trigger it>
- **Environment:** <tool/library versions, OS, runtime that reproduce it>
...
> 배포 워크플로 `pages-deploy.yml`("Build and Deploy")가 빌드 직후 같은 `htmlproofer _site --disable-external …` 검사를 돌립니다(로컬 `tools/test.sh`와 동일한 검사). 단, 문서만 바뀐 push는 `paths-ignore` 때문에 배포되지 않습니다. 빌드·배포 흐름은 [architecture.md §2.3 배포](../../explanation/architecture.md#23-배포) 참고.
